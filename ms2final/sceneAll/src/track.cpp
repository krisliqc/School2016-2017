...
//            int indexR = ((x*scaleX) + videoWidth * (y*scaleY)) * 4 - 3;
//            int indexG = ((x*scaleX) + videoWidth * (y*scaleY)) * 4 - 2;
//            int indexB = ((x*scaleX) + videoWidth * (y*scaleY)) * 4 - 1;
            
            int brightness = fboPixels[index] / distortion;
//            int rValue = fboPixels[indexR] / distortion;
//            int gValue = fboPixels[indexG] / distortion;
//            int bValue = fboPixels[indexB] / distortion;
            
            pts[i].z = brightness;
            
                if(brightness == pts[i].z){
                    
                    mesh.setVertex(i, pts[i]);
                    //            p.z = gValue;
                } else {
                    //                    mesh.setVertex(i, ofPoint(pts[i].x, pts[i].y, 0));pts[i].z = brightness;
                    mesh.setVertex(i, pts[i]);
                    mesh.addColor(ofColor(brightness,0,0,150));
