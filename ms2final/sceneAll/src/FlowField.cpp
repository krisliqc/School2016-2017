...
void FlowField::setPerlin() {
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            float noise = ofMap( ofNoise(x*0.25, y*0.25), 0, 1, 0, TWO_PI);
...
//                flowList[y][x] += flowList[y][x].normalized() * strength;
                ...
                flowList[y][x] += dir.normalize() * strength;
            }
        }
    }
}
    ...
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            ...
            if( np.distance(mousePos) < radius ){
                ...
                // add strength towards the mouse
                ofVec2f dir = (np - mousePos);
                flowList[y][x].x -= dir.normalize().y * strength;
                flowList[y][x].y += dir.normalize().x * strength;
            }
        }
    }
}
...
//	
//	// Draw the cone
//	ofVec3f axis = ( end - start ).normalize();
//    ofVec3f temp = ( axis.dot( ofVec3f(0,1,0) ) > 0.999f ) ? axis.crossed( ofVec3f(1,0,0) ) : axis.cross( ofVec3f(0,1,0) );
//	ofVec3f left = ofVec3f(axis.cross( temp )).normalize();
//	ofVec3f up = axis.cross( left ).normalize();
//    
